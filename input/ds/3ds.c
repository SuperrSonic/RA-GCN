...
static syswd_t alarmb = SYS_WD_NULL;
...
static void timeout_cb(syswd_t alarmb, void *arg)
{
	ctr.data.held = 0;
	ctr.data.touch.x = 0;
	ctr.data.touch.y = 0;
...
void CTRScanPads(void)
{
	...
	while (ctr.sv.sd != INVALID_SOCKET) {
		...
		if (packet.magic == 0x3D5C) {
			...
			SYS_SetAlarm(alarmb, &ctr.tv, timeout_cb, NULL);
		}
	}
