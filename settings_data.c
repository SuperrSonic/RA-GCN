      ...
      char *type_str, size_t type_str_size)
{
  ...
  /* if (!strcmp(setting->name, "video_monitor_index"))
   ...
   else if (!strcmp(setting->name, "aspect_ratio_index") || !strcmp(setting->name, "aspect_ratio_index_wide"))
   {
      strlcpy(type_str,
      ...
      size_t sizeof_msg)
{
 ...
 /*  else if (!strcmp(label, "video_crop_overscan"))
   ...
   else if (!strcmp(label, "video_wide_correction"))
   {
      snprintf(msg, sizeof_msg,
            " -- Set 16:9 scale correction. \n"
            " \n"
            "Scale width up to 720 pixels. \n"
            "This lets you tweak the size to match \n"
            "a certain PAR. \n"
			"Higher quality than bilinear.");
   }
   else if (!strcmp(label, "video_correct_amount"))
   {
      snprintf(msg, sizeof_msg,
            " -- Set VI scaling width. \n"
            " \n"
            "Scale width up to 720 pixels. \n"
            "Only affects game display. \n"
            "Limit is framebuffer width to 720. \n"
			"It can also be used to correct PAR \n"
			"in widescreen correction mode.");
   }
   else if (!strcmp(label, "video_viwidth") || !strcmp(label, "video_wide_viwidth"))
   {
      snprintf(msg, sizeof_msg,
            " -- Set VI scaling width. \n"
            " \n"
...
#ifdef USE_TILED
   else if (!strcmp(label, "video_tiled"))
   {
      snprintf(msg, sizeof_msg,
            " -- Use full SD resolution. \n"
            " \n"
            "Render more pixels by splitting \n"
            "the output in tiles, currently \n"
            "menu transparency not supported. \n");
   }
#endif
   ...
   else if (!strcmp(label, "video_blendframe"))
   {
      snprintf(msg, sizeof_msg,
            ...
            "The doubling is wrong, and requires ON/OFF \n"
            "all tex filtering to avoid incorrect scale. \n"
			" \n"
            "Copying of the previous frame causes a slight\n"
            "performance drop. \n"
			" \n"
            " Default: OFF");
   }
   ...
   else if (!strcmp(label, "video_prescale"))
   {
      snprintf(msg, sizeof_msg,
            " -- Frame prescaling. \n"
            " \n"
            "Oversamples the image. \n"
            "When applying bilinear filtering, \n"
            "the result is a very sharp image. \n"
            "Doesn't affect performance. \n"
			" \n"
            " Default: OFF");
   }
   else if (!strcmp(label, "video_forcesync"))
   {
      snprintf(msg, sizeof_msg,
            " -- Sync video always. \n"
            " \n"
            "This ensures sync to video is always \n"
            "active, thus preventing cores from \n"
            "reporting faulty timings. \n"
            "If speed is below 60, FPS may halve. \n"
			" \n"
            " Default: OFF");
   }
 ...
 /*  else if (!strcmp(label, "savestate_directory"))
...
#ifdef HW_RVL
   else if (!strcmp(label, "input_n3ds"))
   {
      snprintf(msg, sizeof_msg,
            " -- 3DS Controller\n"
            " \n"
			"After enabling, \n"
			"and after content loads, \n"
  ...
  /* else if (!strcmp(label, "grab_mouse_toggle"))
      ...
      snprintf(msg, sizeof_msg,
            " -- Input Device Type. \n"
            " \n"
            "Picks which device type to use. \n"
            "Relevant for the core itself."
            );
   else
      snprintf(msg, sizeof_msg,
            "-- No info. --\n");
...
static void general_write_handler(void *data)
{
   ...
   else if (!strcmp(setting->name, "video_viwidth") || !strcmp(setting->name, "video_correct_amount") ||
            !strcmp(setting->name, "video_wide_viwidth"))
   {
      if (driver.video_data)
        update_screen_width();
   }
   else if (!strcmp(setting->name, "particle_type"))
   {
	   fb_size_changed = true;
   }
#ifdef USE_TILED
   else if (!strcmp(setting->name, "video_tiled"))
   {
	  // Avoids the glitches when using transparency.
	  g_settings.menu_solid = true;
	  
      if (driver.video_data && g_settings.video.tiled)
        gx_set_video_mode(driver.video_data, 720,
            menu_gx_resolutions[g_settings.video.vres][1]);
		else if (driver.video_data) {
   ...
   else if (!strcmp(setting->name, "video_dither") || !strcmp(setting->name, "video_prescale") 
         || !strcmp(setting->name, "video_force_288p"))
   {
      if (driver.video_data) {
      if(!strcmp(setting->name, "video_prescale")) {
         update_prescale();
		 //switching to 240p fixes glitching
		/* gx_set_video_mode(driver.video_data, menu_gx_resolutions
...
static bool START_GROUP(rarch_setting_t **list, rarch_setting_info_t *list_info, rarch_setting_group_info_t group_info, const char* NAME)
{
...
static bool END_GROUP(rarch_setting_t **list, rarch_setting_info_t *list_info)
{
...
static bool START_SUB_GROUP(rarch_setting_t **list, rarch_setting_info_t *list_info, const char *NAME, const char *group_info, rarch_setting_group_info_t subgroup_info)
{
...
static bool END_SUB_GROUP(rarch_setting_t **list, rarch_setting_info_t *list_info)
{
...
static bool CONFIG_ACTION(rarch_setting_t **list, rarch_setting_info_t *list_info, const char* NAME, const char *SHORT, rarch_setting_group_info_t *group_info, rarch_setting_group_info_t *subgroup_info)
{
   if (!settings_list_append(list, list_info, setting_data_action_setting  (NAME, SHORT, group_info->name, subgroup_info->name))) return false;
   
   return false;
}

static bool CONFIG_BOOL(rarch_setting_t **list, rarch_setting_info_t *list_info, bool *TARGET, const char* NAME, const char* SHORT, bool DEF_C, const char* OFF_C, const char* ON_C, rarch_setting_group_info_t *group_info, rarch_setting_group_info_t *subgroup_info, change_handler_t CHANGE_HANDLER, change_handler_t READ_HANDLER)
{
   if (!settings_list_append(list, list_info, setting_data_bool_setting  (NAME, SHORT, TARGET, DEF_C, OFF_C, ON_C, group_info->name, subgroup_info->name, CHANGE_HANDLER, READ_HANDLER)))return false;
   
   return false;
}

static bool CONFIG_INT(rarch_setting_t **list, rarch_setting_info_t *list_info, int *TARGET, const char* NAME, const char* SHORT, int DEF, rarch_setting_group_info_t *group_info, rarch_setting_group_info_t *subgroup_info, change_handler_t CHANGE_HANDLER, change_handler_t READ_HANDLER)
{
   if (!(settings_list_append(list, list_info, setting_data_int_setting   (NAME, SHORT, TARGET, DEF, group_info->name, subgroup_info->name, CHANGE_HANDLER, READ_HANDLER)))) return false;
   
   return false;
}

static bool CONFIG_UINT(rarch_setting_t **list, rarch_setting_info_t *list_info, unsigned int *TARGET, const char *NAME, const char *SHORT, unsigned int DEF, rarch_setting_group_info_t *group_info, rarch_setting_group_info_t *subgroup_info, change_handler_t CHANGE_HANDLER, change_handler_t READ_HANDLER)
{
   if (!(settings_list_append(list, list_info, setting_data_uint_setting  (NAME, SHORT, TARGET, DEF, group_info->name, subgroup_info->name, CHANGE_HANDLER, READ_HANDLER)))) return false;
   
   return false;
}

static bool CONFIG_FLOAT(rarch_setting_t **list, rarch_setting_info_t *list_info, float *TARGET, const char* NAME, const char* SHORT, float DEF, const char* ROUNDING, rarch_setting_group_info_t *group_info, rarch_setting_group_info_t *subgroup_info, change_handler_t CHANGE_HANDLER, change_handler_t READ_HANDLER)
{
   if (!(settings_list_append(list, list_info, setting_data_float_setting (NAME, SHORT, TARGET, DEF, ROUNDING, group_info->name, subgroup_info->name, CHANGE_HANDLER, READ_HANDLER)))) return false;
   
   return false;
}

static bool CONFIG_PATH(rarch_setting_t **list, rarch_setting_info_t *list_info, char *TARGET, const char *NAME, const char *SHORT, const char *DEF, rarch_setting_group_info_t *group_info, rarch_setting_group_info_t *subgroup_info, change_handler_t CHANGE_HANDLER, change_handler_t READ_HANDLER)
{
   if (!(settings_list_append(list, list_info, setting_data_string_setting(ST_PATH, NAME, SHORT, TARGET, 256, DEF, "", group_info->name, subgroup_info->name, CHANGE_HANDLER, READ_HANDLER)))) return false;
   
   return false;
}

static bool CONFIG_DIR(rarch_setting_t **list, rarch_setting_info_t *list_info, char *TARGET, const char *NAME, const char *SHORT, const char *DEF, const char *EMPTY, rarch_setting_group_info_t *group_info, rarch_setting_group_info_t *subgroup_info, change_handler_t CHANGE_HANDLER, change_handler_t READ_HANDLER)
{
   if (!(settings_list_append(list, list_info, setting_data_string_setting(ST_DIR, NAME, SHORT, TARGET, 256, DEF, EMPTY, group_info->name, subgroup_info->name, CHANGE_HANDLER, READ_HANDLER)))) return false;
   
   return false;
}

static bool CONFIG_STRING(rarch_setting_t **list, rarch_setting_info_t *list_info, char *TARGET, const char *NAME, const char *SHORT, const char *DEF, rarch_setting_group_info_t *group_info, rarch_setting_group_info_t *subgroup_info, change_handler_t CHANGE_HANDLER, change_handler_t READ_HANDLER)
{
   if (!(settings_list_append(list, list_info, setting_data_string_setting(ST_STRING, NAME, SHORT, TARGET, sizeof(TARGET), DEF, "", group_info->name, subgroup_info->name, CHANGE_HANDLER, READ_HANDLER)))) return false;
...
static bool CONFIG_BIND(rarch_setting_t **list, rarch_setting_info_t *list_info, struct retro_keybind *TARGET, uint32_t PLAYER, uint32_t PLAYER_OFFSET, const char *NAME, const char *SHORT, const struct retro_keybind *DEF, rarch_setting_group_info_t *group_info, rarch_setting_group_info_t *subgroup_info)
{
   if (!(settings_list_append(list, list_info, setting_data_bind_setting  (NAME, SHORT, TARGET, PLAYER, PLAYER_OFFSET, DEF, group_info->name, subgroup_info->name)))) return false;
      ...
      rarch_setting_info_t *list_info)
{
   ...
   START_GROUP(list, list_info, group_info, "Main Menu");
      ...
      rarch_setting_info_t *list_info)
{
   ...
   START_GROUP(list, list_info, group_info, "Menu");
...
#if 1
   START_GROUP(list, list_info, group_info, "General");
   settings_data_list_current_add_flags(list, list_info, SD_FLAG_IS_CATEGORY);
   START_SUB_GROUP(list, list_info, "State", group_info.name, subgroup_info);
      ...
      rarch_setting_info_t *list_info)
{
   ...
   START_GROUP(list, list_info, group_info, "Video");
   settings_data_list_current_add_flags(list, list_info, SD_FLAG_IS_CATEGORY);
   START_SUB_GROUP(list, list_info, "State", group_info.name, subgroup_info);
...
/* Added for testing */
  ...
  */
...
#ifdef HW_RVL
   CONFIG_BOOL(list, list_info,
         g_settings.video.wide_correction,
         "video_wide_correction",
   ...
   CONFIG_UINT(list, list_info,
         g_settings.video.correct_amount,
         "video_correct_amount",
         "Correction Amount",
         video_correct_amount,
         &group_info,
         &subgroup_info,
         general_write_handler,
         general_read_handler);
   settings_list_current_add_range(list, list_info, 256, 720, 2, true, true);
   ...
   CONFIG_UINT(list, list_info,
         ...
         general_write_handler,
         general_read_handler);
   settings_list_current_add_range(list, list_info, gx_mode.fbWidth, 720, 2, true, true);
#ifdef HW_RVL
   } else {
   CONFIG_UINT(list, list_info,
         g_settings.video.wide_viwidth,
         "video_wide_viwidth",
         "Screen Width",
         video_wide_viwidth,
         &group_info,
      ...
      rarch_setting_info_t *list_info)
{
   ...
   START_GROUP(list, list_info, group_info, "Audio");
   settings_data_list_current_add_flags(list, list_info, SD_FLAG_IS_CATEGORY);
   START_SUB_GROUP(list, list_info, "State", group_info.name, subgroup_info);
  ...
  /* CONFIG_BOOL(
   ...
   CONFIG_UINT(list, list_info,
         ...
         general_write_handler,
         general_read_handler);
	settings_list_current_add_range(list, list_info, 2, 64, 2, true, true);
      ...
      rarch_setting_info_t *list_info)
{
   ...
   START_GROUP(list, list_info, group_info, "Input");
   settings_data_list_current_add_flags(list, list_info, SD_FLAG_IS_CATEGORY);
   START_SUB_GROUP(list, list_info, "State", group_info.name, subgroup_info);
  ...
  CONFIG_UINT(list, list_info,
   ...
   CONFIG_UINT(list, list_info,
         ...
         general_write_handler,
         general_read_handler);
   settings_list_current_add_range(list, list_info, 0, 60, 1, true, true);
   ...
   CONFIG_UINT(list, list_info,
         ...
         "input_n3ds_stick_right",
         "3DS Key Circle Right",
         n3ds_stick_right,
         &group_info,
         &subgroup_info,
         general_write_handler,
   ...
   for (i = 0; i < RARCH_BIND_LIST_END; i ++)
   {
      ...
      CONFIG_BIND(list, list_info, &g_settings.input.binds[0][i], 0, 0,
            keybind->base, keybind->desc, &retro_keybinds_1[i],
            &group_info, &subgroup_info);
      ...
      rarch_setting_info_t *list_info)
{
   ...
   START_GROUP(list, list_info, group_info, "Overlays");
   settings_data_list_current_add_flags(list, list_info, SD_FLAG_IS_CATEGORY);
   START_SUB_GROUP(list, list_info, "State", group_info.name, subgroup_info);
...
#if 0 //broken after coord changes?
   CONFIG_FLOAT(list, list_info,
      ...
      rarch_setting_info_t *list_info)
{
   ...
   START_GROUP(list, list_info, group_info, "Paths");
   settings_data_list_current_add_flags(list, list_info, SD_FLAG_IS_CATEGORY);
   START_SUB_GROUP(list, list_info, "State", group_info.name, subgroup_info);
