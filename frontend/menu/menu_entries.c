      ...
      unsigned default_type_plain, const char *exts)
{
...
#ifdef USE_NAND
      menu_list_push(list,
            "nand:/", "", MENU_FILE_DIRECTORY, 0);
#endif
#endif
#ifdef HW_DOL
      menu_list_push(list,
            "gcsd:/", "", MENU_FILE_DIRECTORY, 0);
#ifdef USE_SP2
      menu_list_push(list,
            "sp2:/", "", MENU_FILE_DIRECTORY, 0);
#endif
#endif
      menu_list_push(list,
   ...
   for (i = 0; i < str_list->size; i++)
   {
      ...
      //char *path_c = str_list->elems[i].data;
     // if(g_settings.single_mode) // Don't show extensions
        // strip_ext(path_c);
