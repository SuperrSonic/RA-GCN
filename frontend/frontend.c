...
#ifdef USE_TITLE
#include "../wii/utils/playlog.h" // for vc settings
#endif
...
void main_exit(args_type() args)
{
   ...
   if (g_settings.single_mode) {
      ...
      // save vc settings.sav if changed
      s32 fd = -1;
      fd = IOS_Open(_vcsettings_path, IPC_OPEN_RW);
      if(fd < 0) {
         ; // oh well
      } else {
         vcSettings * vc_buf = memalign(32, ALIGN32(sizeof(vcSettings)));
         if(vc_buf) {
            if(IOS_Read(fd, vc_buf, sizeof(vcSettings)) == sizeof(vcSettings)) {
               if(IOS_Seek(fd, 0, 0) > -1) {
                  // Using one of the empty slots for 3DS ctroller, only up to 2 players for now
                  if(vc_buf->n3ds_port != g_settings.input.n3ds_port) {
                    vc_buf->n3ds_port = g_settings.input.n3ds_port;
                    IOS_Write(fd, vc_buf, sizeof(vcSettings));
                  }
               }
            }
            free(vc_buf);
  ...
  // rarch_main_command(RARCH_CMD_PERFCNT_REPORT_FRONTEND_LOG);
   ...
   // trying to fix visual noise when exiting
#ifdef USE_NAND
   ISFS_Unmount();
   ISFS_Deinitialize();
